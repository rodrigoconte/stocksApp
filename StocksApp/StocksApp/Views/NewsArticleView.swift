                ...
                VStack {
                    ForEach(self.newsArtices, id: \.title) { article in
                        HStack {
                            ...
                            URLImage(URL(string: article.imageURL)!, content: { $0.resizable() })
...
struct NewsArticleView_Previews: PreviewProvider {
    static var previews: some View {
        ...
        return NewsArticleView(newsArtices: [ArticleViewModel(article: article)], onDragBegin: { _ in }, onDragEnd: { _ in } )
    }
}
